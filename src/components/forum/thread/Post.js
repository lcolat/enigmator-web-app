...
import {Paper, Typography, Divider, IconButton, TextField, Tooltip} from '@material-ui/core';

import {PlaylistAdd, PlaylistAddCheck, ArrowDropDown, ArrowDropUp} from "@material-ui/icons";
		...
		<Paper className={classes.root}>
			<Grid container>
						...
						<Grid container direction={"row"} justify={"space-between"}>
							<Grid item>
								<Grid container direction={"row"} alignItems={"flex-end"}>
									<Avatar alt={postData.creator.name}
									        src={postData.creator.avatar ? postData.creator.avatar :
							...
							<LikeCount likes={postData.like.number} liked={postData.like.byUser}/>
						</Grid>
					</Grid>
				...
				<Grid container direction={"row"} justify={"space-between"} alignItems={"center"}>
					...
					<div>
						<Grid container direction={"row"} alignItems={"center"}>
							<Typography className={classes.typoComment}>
								Comments
							</Typography>
							<Typography>
					...
					<IconButton color="primary" className={classes.button} aria-label="Drop Arrow"
					            onClick={changeArrow}>
				...
				<Grid item className={classes.comments}>
					<Grid container direction={"column"}>
						{commentUp && postData.comment.map(com => (
							<Grid item className={classes.comment} key={`${com.name}-${com.date}`}>
								<Grid container direction={"column"}>
									<Divider style={{marginBottom: 3}}/>
									<Grid item>
										<Grid container direction={"row"} justify={"space-between"}>
											<Grid item>
												<Grid container direction={"row"} alignItems={"flex-end"}>
													<Typography variant={"body2"}>{com.name}</Typography>
													<Typography variant={"caption"} style={{marginLeft: 7}}>
														{com.date}
													</Typography>
												</Grid>
											</Grid>
											<LikeCount liked={com.like.byUser} likes={com.like.number}/>
										</Grid>
									</Grid>
