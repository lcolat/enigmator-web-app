		...
		<Paper>
			...
			{settingGroupEnable &&
				settingsGroups.map(settingGroup => (
					<SettingChoice
						groupName={settingGroup.groupName}
			...
			<Grid
				...
				className={classes.rootTypoChange}>
				...
				<FormControl className={classes.formControl}>
					<Select
						...
						input={
							<Input name="typography" id="typography-label-placeholder" />
						}
						name="Enigmator Typography"
						displayEmpty>
						{listTypo.map((typo, index) =>
							index === 0 ? (
								<MenuItem value="" key={index + '-' + typo}>
									<em>{typo}</em>
								</MenuItem>
							) : (
								<MenuItem value={index} key={index + '-' + typo}>
									{typo}
								</MenuItem>
							)
						)}
